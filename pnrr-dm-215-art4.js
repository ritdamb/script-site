(function () {
  ...
  fetch(ENDPOINT)
    .then(r => {
      console.log('[PNRR 215 ART. 4] fetch status:', r.status);
      return r.json();
    })
    .then(data => {
      const items = data.items || [];
      console.log('[PNRR 215 ART. 4] avvisi totali nel feed:', items.length);

      const filtrati = items.filter(item => {
        const testo = ((item.titolo || '') + ' ' + (item.descrizione || ''));
        return /PNRR/i.test(testo) && /\bDM\s+215\b/i.test(testo) && /\bART\.?\s*4\b/i.test(testo);
      });

      filtrati.sort((a, b) => (b.lastModifiedAt || 0) - (a.lastModifiedAt || 0));

      console.log('[PNRR 215 ART. 4] avvisi corrispondenti:', filtrati.length);
    ...
    .catch(err => {
      console.error('[PNRR 215 ART. 4] fetch fallito:', err);
